...
// Removes the specific "Magenta" background generated by Gemini
const processChromaKey = (base64: string): Promise<string> => {
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            // We use Euclidean distance in RGB space to detect this color and remove it.
            const targetR = 255;
            const targetG = 0;
            const targetB = 255;
            
            // Tolerance: how strictly exact the color must be (0-441)
            // 120 allows for some compression artifacts/lighting changes on the background
            const tolerance = 120; 
            
            // Smoothing: Feather the edge (pixels between tolerance and tolerance+smoothing)
            const smoothing = 60;
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                if (distance < tolerance) {
                    // Exact match (or close enough) -> Transparent
                    data[i + 3] = 0;
                } else if (distance < tolerance + smoothing) {
                    // Edge pixel -> Semi-transparent (Anti-aliasing)
                    // Linear interpolation from 0 to 255
                    const alpha = (distance - tolerance) / smoothing;
                    data[i + 3] = Math.floor(alpha * 255);
                    
                    // Despill: If it was reddish-purple, shift it slightly to neutral to remove halo
                    if (r > 200 && b > 200) {
                        const avg = (r + g + b) / 3;
                        data[i] = avg;     // R
                        data[i+1] = avg;   // G
...
export const removeBackgroundImage = async (
    ...
    try {
        ...
        // --- STRATEGY: SMART CHROMA KEY ---
        if (apiKey) {
            updateStatus("⚡ NETWORK: Gemini 2.5 Flash (Chroma Engine)");
            ...
            // We ask Gemini to output the object on a solid Magenta background.
            // This is easier for the AI than generating a perfect transparent PNG directly,
            // and easier for us to key out than a complex mask.
            const prompt = "Extract the main subject from the image. Place the subject on a pure solid #FF00FF (Magenta) background. Ensure the edges are sharp and clean. Do not alter the subject's colors.";
